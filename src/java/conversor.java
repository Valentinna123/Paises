...
public class conversor extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int valor=Integer.parseInt(request.getParameter("p3")) ;
       
            if(pais.equals("Madagascar")){
                cMadagascar ob= new cMadagascar();
                
                out.println(ob.conversor(valor)+" Ariary Malgache");
                
            }else if(pais.equals("Marruecos")){
                dMarruecos ob= new  dMarruecos();
                
               out.println(ob.conversor(valor)+" Dirham Marroquí");
                
            }else if(pais.equals("Chad")){
                 eChad ob= new  eChad();
                
                 out.println(ob.conversor(valor)+" Franco CFA de África Central");
               
            }else if(pais.equals("Costa de Marfil")){
                fCostaDeMarfil ob= new   fCostaDeMarfil();
                
                 out.println(ob.conversor(valor)+" Franco CFA de África Occidental");
